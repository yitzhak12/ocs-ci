...
class VSPHERE(object):
    ...
    def get_disk_sizes(self, vm):
        """
        Retrieves the sizes of all disks attached to a given VM instance in gigabytes (GB).

        Args:
            vm (vim.VirtualMachine): VM instance.

        Returns:
            list: A list containing the sizes of the disks in GB.

        """
        disks = self.get_disks(vm)

        if not disks:
            logger.warning(f"No disks found for VM: {vm.config.name}")
            return []

        disk_sizes = []
        for disk in disks:
            size_kb = disk.get("capacityInKB", 0)
            if size_kb == 0:
                logger.warning(
                    f"Disk {disk.get('deviceName', 'unknown')} has no valid size"
                )
            disk_sizes.append(size_kb // GB2KB)
